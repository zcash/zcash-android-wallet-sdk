...
mod live_wallet_tests {
    ...
    fn build_and_finalize_all_unsigned() {
        ...
        let (fvk, spendable) = {
            let backend = Backend::new(&wallet, account, None, &mut store_conn).expect("Backend::new");
        ...
        let (state, unsigned) = {
            let mut backend = Backend::new(&wallet, account, None, &mut store_conn).expect("Backend::new");
            let mut rng = OsRng;
...
mod live_wallet_signing_tests {
    ...
    fn commit_and_finalize_with_real_signing() {
        ...
        let state = {
            let mut backend = Backend::new(&wallet, account, Some(usk), &mut store_conn).expect("Backend::new");
            let mut rng = OsRng;
...
mod live_wallet_edge_case_tests {
    ...
    fn singleton_id_collision_between_accounts() {
        ...
        let target = tip + 1;
        {
            let mut backend_a = Backend::new(&wallet, account_a, None, &mut store_conn).expect("Backend::new");
            let mut rng = OsRng;
        ...
        let backend_b = Backend::new(&wallet, account_b, None, &mut store_conn).expect("Backend::new");
    ...
    fn migration_state_persists_across_reopened_connection() {
        ...
        let backend2 = Backend::new(&wallet2, account, None, &mut store_conn2).expect("Backend::new");
    ...
    fn planning_an_account_with_no_funds_errors_cleanly() {
        ...
        let backend = Backend::new(&wallet, account_b, None, &mut store_conn).expect("Backend::new");
        let mut rng = OsRng;
