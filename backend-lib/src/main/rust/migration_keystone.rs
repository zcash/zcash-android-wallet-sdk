...
pub fn build_sign_batch_qr_parts(
    ...
    if let Some(split) = split_unsigned {
        let parsed =
            pczt::parse(split).map_err(|e| anyhow::anyhow!("parse split pczt: {e:?}"))?;
...
mod tests {
    ...
    fn build_sign_batch_qr_parts_strips_preexisting_orchard_spend_auth_sig() {
        ...
        decoder
            ...
            .expect("ur receive");
        assert!(decoder.complete(), "single fragment should complete decoding");
        let cbor = decoder
            .message()
            .expect("ur message")
        ...
        let request =
            BatchSignRequest::parse(batch.get_data()).expect("parse batch sign request");
