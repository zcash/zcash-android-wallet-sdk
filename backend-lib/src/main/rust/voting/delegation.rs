...
fn build_governance_pczt_for_bundle(
    ...
    let pczt = db
        .build_governance_pczt(round_id, bundle_index, &bundle_notes, &keys, consensus_branch_id)
...
>(
    ...
    let res = catch_unwind(&mut env, |env| {
        ...
        let result = db
            .precompute_delegation_pir(&round_id, bundle_index, &bundle_notes, &pir_client, db.network)
...
>(
    ...
    let res = catch_unwind(&mut env, |env| {
        ...
        let result = db
            .build_and_prove_delegation(&round_id, bundle_index, &bundle_notes, &keys, &pir_client, &stages)
...
fn gather_delegation_keys_for_submission(
    ...
    .map(
        |summary: zcash_client_backend::data_api::WalletSummary<zcash_client_sqlite::AccountUuid>| {
...
mod tests {
    ...
    fn extract_spend_auth_sig_accepts_signed_governance_pczt() {
        ...
        let account_usk =
            UnifiedSpendingKey::from_seed(&Network::TestNetwork, &account_seed, zip32::AccountId::ZERO)
                .expect("account USK");
